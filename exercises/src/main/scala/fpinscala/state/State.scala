...
object Candy {
  ...
  def simulateMachine(inputs: List[Input]): State[MachineState, (Int, Int)] = for {

    _ <- sequence(inputs.map(i => modify((s: MachineState) => (i, s) match {

      case (_, MachineState(_, 0, _)) => s
      case (Coin, MachineState(false, _, _)) => s
      case (Turn, MachineState(true, _, _)) => s
      case (Coin, MachineState(true, candy, coin)) =>
        MachineState(false, candy, coin + 1)
      case (Turn, MachineState(false, candy, coin)) =>
        MachineState(true, candy - 1, coin)
    })))
    s <- get
