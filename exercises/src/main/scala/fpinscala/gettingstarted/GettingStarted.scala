...
object PolymorphicFunctions {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ascendArray: Array[Int] = Array(1, 2, 3, 4, 5, 6)

    def lteq[A](a: A, b: A)(implicit ordering: Ordering[A]): Boolean = ordering.lteq(a, b)
    def gt[A](a:A, b:A) (implicit ordering: Ordering[A]): Boolean = ordering.gt(a,b)

    // test
    val bool: Boolean = isSorted(ascendArray, lteq[Int])
    println(bool)

    val binarySearchResult : Int = binarySearch(ascendArray, 3, gt[Int])
