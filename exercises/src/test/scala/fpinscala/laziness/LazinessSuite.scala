...
class LazinessSuite extends FunSuite {
  ...
  test ("Stream(1,2,3,4).takeWhile2( _ > 2)"){
    assert(Stream(1,2,3,4).takeWhile2(_ > 2).exists(_ == 3) === true)
    assert(Stream(1,2,3,4).takeWhile2(_ > 2).exists(_ == 4) === true)
    assert(Stream(1,2,3,4).takeWhile2(_ > 2).exists(_ == 2) === false)
